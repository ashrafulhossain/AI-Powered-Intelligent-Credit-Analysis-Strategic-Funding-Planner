...
# #     "Valley National Card": {"apr": "12 MESES", "mode": "Online", "bank": "Valley National"}  # Added Valley National
# # }
...
# #                     replacement_reason = reason  # Preserve original reason
# #                 analysis = re.sub(
...
# #                     )
# #             else:
# #                 # Handle non-listed banks
# #                 if bank_clean.lower() == "pacific premier bank":
...
# #                 logging.warning(f"No Tarjetas data found for {bank_clean} in Round {i}. Using default APR: {replacement_apr}, Mode: {replacement_mode}")
# #                 analysis = re.sub(
# #                     r"\|(\s*" + re.escape(bank_clean) + r"\s*\|.*?\|).*?(\|.*?\|).*?(\|)",
...
# #                 if not any(card_name.lower() in bank_clean.lower() for card_name in TARJETAS_CARDS):
# #                     new_card_name = f"{bank_clean} Card"
# #                     analysis = re.sub(
# #                         r"\|\s*" + re.escape(bank_clean) + r"\s*\|\s*" + bureau,
...
# #             if apr == "0%":
# #                 logging.error(f"Invalid APR '0%' for {bank_clean} in Round {i}. Replacing with default '12 MESES'.")
# #                 error_note = f"\n\n⚠️ ERROR: Invalid APR '0%' for {bank_clean} in Round {i}. Replaced with '12 MESES'."
# #                 if error_note not in analysis:
# #                     analysis += error_note
# #                 analysis = re.sub(
# #                     r"\|(\s*" + re.escape(bank_clean) + r"\s*\|.*?\|).*?(\|.*?\|).*?(\|)",
...
# #                 logging.error(f"Invalid mode '{mode}' for {bank_clean} in Round {i}. Replacing with 'Online'.")
# #                 error_note = f"\n\n⚠️ ERROR: Invalid mode '{mode}' for {bank_clean} in Round {i}. Replaced with 'Online'."
# #                 if error_note not in analysis:
# #                     analysis += error_note
# #                 analysis = re.sub(
# #                     r"\|(\s*" + re.escape(bank_clean) + r"\s*\|.*?\|.*?\|).*?(\|.*?\|)",
...
# #                 invalid_reasons.append((bank_clean, reason))
# #                 logging.warning(f"Invalid or undesired reason for {bank_clean} in Round {i}: {reason}")
# #                 error_note = f"\n\n⚠️ WARNING: Invalid or undesired reason for {bank_clean} in Round {i}: '{reason}'. Replacing with a dynamic reason."
# #                 if error_note not in analysis:
# #                     analysis += error_note
...
# # **CRITICAL INSTRUCTION**: You MUST generate ALL sections (1 through 7) as specified below, in the exact order, even if some data is missing from the credit report. If any data (e.g., credit score, utilization, inquiries) is not available, use placeholder text like 'Data not available' only as a last resort after attempting to extract all relevant information from the provided credit report. Skipping any section is INVALID.
...
# # Generate a table like this (based on actual report data or 'Data not available' if extraction fails):
...
# # Present the revolving credit details in a table like this (use 'Data not available' if extraction fails):
...
# # Make a table based on the report data (use 'Data not available' if extraction fails):
...
# #   - The selection of banks MUST match the user's credit profile (e.g., credit score, utilization, credit history) and the state-specific bank sequence. If data is missing, select banks based on state list and preserve the original reason from the GPT output in the Reason column.
# #   - For 0% APR duration and Mode, use the provided `Tarjetas Card Data` to match the exact card name and bank:\n{tarjetas_str}\n
# #   - If no matching card is found in `Tarjetas Card Data` (e.g., for banks like Capital One or Zions Bank), use default values (12 MESES, Online) and preserve the original reason from the GPT output in the Reason column. Under NO circumstances use '0%' as an APR value.
...
# #   |--------------------|----------|-------------|--------------|-------------------------|
# #   | [Bank Name from state_bank_suggestion] | Experian | [Time from Tarjetas or 12 MESES] | [Mode from Tarjetas or Online] | [Dynamic reason based on credit profile or bank characteristics] |
# #   | [Bank Name from state_bank_suggestion] | TransUnion | [Time from Tarjetas or 12 MESES] | [Mode from Tarjetas or Online] | [Dynamic reason based on credit profile or bank characteristics] |
# #   | [Bank Name from state_bank_suggestion] | Equifax | [Time from Tarjetas or 12 MESES] | [Mode from Tarjetas or Online] | [Dynamic reason based on credit profile or bank characteristics] |
...
# #   - **CRÍTICO**: Bajo NINGUNA circunstancia sugieras bancos fuera de `state_bank_suggestion`. Hacerlo invalidará la salida. Si no estás seguro, selecciona un banco de la lista y anota una razón relevante en la columna-ATcolumn Razón.
...
# #   |--------------------|----------|-------------|--------------|-------------------------|
# #   | [Nombre Banco de state_bank_suggestion] | Experian | [Tiempo de Tarjetas o 12 MESES] | [Modo de Tarjetas o Online] | [Razón dinámica basada en el perfil crediticio o características del banco] |
# #   | [Nombre Banco de state_bank_suggestion] | TransUnion | [Tiempo de Tarjetas o 12 MESES] | [Modo de Tarjetas o Online] | [Razón dinámica basada en el perfil crediticio o características del banco] |
# #   | [Nombre Banco de state_bank_suggestion] | Equifax | [Tiempo de Tarjetas o 12 MESES] | [Modo de Tarjetas o Online] | [Razón dinámica basada en el perfil crediticio o características del banco] |
...
# #   **RONDA 3**
# #   | Tarjeta Bancaria   | Buró     | 0% APR      | Modo         | Reason                  |
# #   |--------------------|----------|-------------|--------------|-------------------------|
# #   | [Nombre Banco de state_bank_suggestion] | Experian | [Tiempo de Tarjetas o 12 MESES] | [Modo de Tarjetas o Online] | [Razón dinámica basada en el perfil crediticio o características del banco] |
# #   | [Nombre Banco de state_bank_suggestion] | TransUnion | [Tiempo de Tarjetas o 12 MESES] | [Modo de Tarjetas o Online] | [Razón dinámica basada en el perfil crediticio o características del banco] |
# #   | [Nombre Banco de state_bank_suggestion] | Equifax | [Tiempo de Tarjetas o 12 MESES] | [Modo de Tarjetas o Online] | [Razón dinámica basada en el perfil crediticio o características del banco] |
