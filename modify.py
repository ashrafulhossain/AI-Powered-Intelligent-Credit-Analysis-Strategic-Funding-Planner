...
# def validate_gpt_output(analysis, state_cards, user_state, json_data, mode="free"):
#     """
#     Validates and corrects the GPT-generated credit analysis output to ensure compliance with rules.
#     Prevents duplicate banks across rounds, ensures cards are from state_cards, and validates APR, mode, and bureau.
#     """
#     import re
#     import logging
...
#         # Initialize tracking for banks and travel cards
#         used_banks_all_rounds = set()
#         used_non_travel_banks = set()
#         travel_card_used = {}
#         valid_modes = ["Online", "In-branch", "Phone", "Online (requires account)", "Online (Omaha Zip)", "Phone/In-branch", "In-branch/Phone"]
#         preferred_travel_cards = [
#             'Chase Sapphire Preferred',
#             'BOFA Alaska Airlines Business',
#             'Amex Delta SkyMiles Gold Business'
#         ]

#         # Extract rounds from the analysis
#         rounds = re.findall(r"\*\*ROUND [1-3]\*\*(.*?)(?=\*\*ROUND|\*\*Strategic Insights|\Z)", analysis, re.DOTALL)
...
#             for row in rows:
#                 card_name, bureau, apr, mode, reason = [x.strip() for x in row]
#                 card_info = json_data.get(card_name)

#                 # Check if card is valid and in state_cards
#                 if not card_info or card_name not in [card['card_name'] for card in state_cards]:
#                     invalid_cards.append(card_name)
#                     logging.error(f"Invalid card {card_name} in Round {i}. Not found in JSON data or state-specific list.")
#                     error_note = f"\n\n⚠️ ERROR: Card {card_name} in Round {i} is not in JSON data or not available in {user_state}. Replacing with a valid card."
#                     if error_note not in analysis:
#                         analysis += error_note

#                     replacement_card = None
#                     # Prioritize travel cards
#                     for travel_card in preferred_travel_cards:
#                         for card in state_cards:
#                             if (card['card_name'] == travel_card and
#                                 card['card_name'] not in banks and
#                                 card['bureau'] == bureau and
#                                 card['bank'] not in used_banks_all_rounds and
#                                 not travel_card_used.get(card['bank'], False)):
#                                 replacement_card = card
#                                 break
#                         if replacement_card:
#                             break
#                     # If no travel card, select any valid card
#                     if not replacement_card:
#                         for card in state_cards:
#                             if (card['card_name'] not in banks and
#                                 card['bureau'] == bureau and
#                                 card['bank'] not in used_banks_all_rounds):
#                                 replacement_card = card
#                                 break
...
#                         replacement_reason = "Strong credit history"
#                         analysis = re.sub(
#                             r"\|\s*" + re.escape(card_name) + r"\s*\|\s*" + bureau + r"\s*\|\s*[^\|]*?\s*\|\s*[^\|]*?\s*\|\s*[^\|]*?\s*\|",
#                             f"| {replacement_card['card_name']} | {bureau} | {replacement_apr} | {replacement_mode} | {replacement_reason} |",
#                             analysis
#                         )
#                         card_name = replacement_card['card_name']
#                         card_info = replacement_card
#                     else:
#                         logging.error(f"No valid replacement card found for {card_name} in Round {i}.")
#                         error_note = f"\n\n⚠️ ERROR: No valid card found in JSON data for Round {i}. Please check JSON data."
#                         if error_note not in analysis:
#                             analysis += error_note
#                         continue
...
#                 # Check for duplicate banks across all rounds
#                 if card_bank in used_banks_all_rounds:
#                     logging.error(f"Duplicate bank {card_bank} in Round {i} with card {card_name}. Replacing with a valid card.")
#                     error_note = f"\n\n⚠️ ERROR: Duplicate bank {card_bank} in Round {i} with card {card_name}. Replacing with a valid card."
#                     if error_note not in analysis:
#                         analysis += error_note
#                     replacement_card = None
#                     # Prioritize travel cards
...
#                         replacement_apr = replacement_card['apr']
#                         replacement_mode = replacement_card['mode']
#                         replacement_reason = "Low utilization" if "utilization" in analysis_lower else "Strong credit history"
#                         analysis = re.sub(
#                             r"\|\s*" + re.escape(card_name) + r"\s*\|\s*" + bureau + r"\s*\|\s*[^\|]*?\s*\|\s*[^\|]*?\s*\|\s*[^\|]*?\s*\|",
#                             f"| {replacement_card['card_name']} | {bureau} | {replacement_apr} | {replacement_mode} | {replacement_reason} |",
#                             analysis
...
#                     else:
#                         logging.error(f"No valid replacement card found for {card_name} in Round {i}.")
#                         error_note = f"\n\n⚠️ ERROR: No valid replacement card found for {card_name} in Round {i}. Please check JSON data."
#                         if error_note not in analysis:
#                             analysis += error_note
#                         continue

#                 # Add bank to used set
#                 used_banks_all_rounds.add(card_bank)
#                 if not card_info['is_travel']:
#                     used_non_travel_banks.add(card_bank)
#                 if card_info['is_travel']:
#                     travel_card_used[card_bank] = True

#                 # Validate APR, mode, and bureau
#                 expected_apr = card_info['apr']
#                 expected_mode = card_info['mode']
#                 expected_bureau = card_info['bureau']
#                 if apr != expected_apr:
#                     apr_mismatches.append((card_name, expected_apr, apr))
#                     logging.error(f"APR mismatch for {card_name} in Round {i}: Expected {expected_apr}, Got {apr}")
#                     error_note = f"\n\n⚠️ ERROR: APR for {card_name} in Round {i} is incorrect. Expected {expected_apr}, Got {apr}."
#                     if error_note not in analysis:
#                         analysis += error_note
#                     analysis = re.sub(
#                         r"\|(\s*" + re.escape(card_name) + r"\s*\|.*?\|).*?(\|.*?\|).*?(\|)",
#                         f"|\1{bureau}|{expected_apr}|{mode}|{reason}|",
#                         analysis
#                     )
#                 if mode != expected_mode:
#                     mode_mismatches.append((card_name, expected_mode, mode))
#                     logging.error(f"Mode mismatch for {card_name} in Round {i}: Expected {expected_mode}, Got {mode}")
#                     error_note = f"\n\n⚠️ ERROR: Mode for {card_name} in Round {i} is incorrect. Expected {expected_mode}, Got {mode}."
#                     if error_note not in analysis:
#                         analysis += error_note
#                     analysis = re.sub(
#                         r"\|(\s*" + re.escape(card_name) + r"\s*\|.*?\|.*?\|).*?(\|.*?\|)",
...
#                     )
#                 if bureau != expected_bureau:
#                     bureau_mismatches.append((card_name, expected_bureau, bureau))
#                     logging.error(f"Bureau mismatch for {card_name} in Round {i}: Expected {expected_bureau}, Got {bureau}")
#                     error_note = f"\n\n⚠️ ERROR: Bureau for {card_name} in Round {i} is incorrect. Expected {expected_bureau}, Got {bureau}."
#                     if error_note not in analysis:
#                         analysis += error_note
#                     analysis = re.sub(
#                         r"\|(\s*" + re.escape(card_name) + r"\s*\|).*?(\|.*?\|.*?\|.*?\|)",
...
#                 # Validate APR values
#                 if apr == "0%" or apr == "NO":
#                     logging.error(f"Invalid APR '{apr}' for {card_name} in Round {i}. Replacing with default '12 MESES'.")
#                     error_note = f"\n\n⚠️ ERROR: Invalid APR '{apr}' for {card_name} in Round {i}. Replaced with '12 MESES'."
#                     if error_note not in analysis:
#                         analysis += error_note
#                     analysis = re.sub(
#                         r"\|(\s*" + re.escape(card_name) + r"\s*\|.*?\|).*?(\|.*?\|).*?(\|)",
...
#                 # Validate mode
#                 if mode not in valid_modes:
#                     logging.error(f"Invalid mode '{mode}' for {card_name} in Round {i}. Replacing with 'Online'.")
#                     error_note = f"\n\n⚠️ ERROR: Invalid mode '{mode}' for {card_name} in Round {i}. Replaced with 'Online'."
#                     if error_note not in analysis:
#                         analysis += error_note
#                     analysis = re.sub(
#                         r"\|(\s*" + re.escape(card_name) + r"\s*\|.*?\|.*?\|).*?(\|.*?\|)",
...
#                 reason_valid = any(re.search(pattern, reason.lower()) for pattern in valid_reasons)
#                 if not reason_valid or "missing credit data" in reason.lower():
#                     invalid_reasons.append((card_name, reason))
#                     logging.warning(f"Invalid or undesired reason for {card_name} in Round {i}: {reason}")
#                     error_note = f"\n\n⚠️ WARNING: Invalid or undesired reason for {card_name} in Round {i}: '{reason}'. Replacing with a dynamic reason."
#                     if error_note not in analysis:
#                         analysis += error_note
#                     replacement_reason = "Diversify cards"
#                     if "requires account" in mode.lower():
...
#                 banks.append(card_name)
#                 bureaus.append(bureau)

#             # Validate bureau order and variety
#             if len(set(bureaus)) != 3 or bureaus != ['Experian', 'TransUnion', 'Equifax']:
#                 logging.warning(f"Invalid bureau variety in Round {i}: {bureaus}")
#                 error_note = f"\n\n⚠️ WARNING: Round {i} does not contain exactly one of each bureau (Experian, TransUnion, Equifax) in order."
#                 if error_note not in analysis:
...
#             # Check for duplicate cards in the same round
#             if len(set(banks)) != len(banks):
#                 logging.warning(f"Duplicate cards in Round {i}: {banks}")
#                 error_note = f"\n\n⚠️ WARNING: Round {i} contains duplicate cards."
#                 if error_note not in analysis:
#                     analysis += error_note
...
#         # Add game plans to Strategic Insights
#         strategic_insights = []
#         for round_num, round_content in enumerate(rounds, 1):
#             rows = re.findall(r"\|\s*([^\|]+?)\s*\|\s*(Experian|TransUnion|Equifax)\s*\|\s*([0-9]+ MESES|Default [0-9]+ MESES|0%|\s*NO\s*)\s*\|\s*([^\|]*?)\s*\|\s*([^\|]*?)\s*\|", round_content)
#             for card_name, bureau, apr, mode, reason in rows:
#                 card_name_clean = card_name.strip()
#                 card_info = json_data.get(card_name_clean)
#                 if card_info and card_info['game_plan']:
#                     strategic_insights.append(f"- For {card_name_clean} (Round {round_num}): {card_info['game_plan']}")
...
#             f"**CRITICAL**: In the 'Card Name' column, ALWAYS use the EXACT card name from `Card Data` (e.g., 'BOFA Unlimited Cash'). You MUST NOT use bank names alone (e.g., 'Bank of America') or append 'Card' to a bank name (e.g., 'Capital One Card'). If no matching card is found in `Card Data` for a bank in `state_cards`, exclude that bank and select another card from `state_cards`.\n"
#             f"**CRITICAL**: GPT MUST NOT generate or suggest any card names outside of `Card Data`. Any attempt to create new card names will invalidate the output.\n"
#             f"**CRITICAL**: Each round (R1, R2, R3) and the entire sequence MUST use different banks. No bank can be repeated across the sequence, even in different rounds.\n"  # Added here
#             f"Each round (R1, R2, R3) MUST include EXACTLY 3 different banks, each associated with a different bureau in this order: Experian, TransUnion, Equifax.\n"
#             f"Each table row MUST contain exactly 5 columns: Card Name, Bureau, 0% APR, Mode, and Reason. Missing fields will invalidate the output.\n"
#             f"For 0% APR duration, Mode, and Bureau, use the provided `Card Data` below to match the exact card name and bank:\n{tarjetas_str}\n"
#             f"Only one 0% Chase card is allowed per sequence, unless the second is a co-branded travel/hotel card (verify `is_travel` field in `Card Data`).\n"
...
#   - **CRITICAL**: In the 'Card Name' column, ALWAYS use the EXACT card name from `Card Data` (e.g., 'BOFA Unlimited Cash'). You MUST NOT use bank names alone (e.g., 'Bank of America') or append 'Card' to a bank name (e.g., 'Capital One Card'). If no matching card is found in `Card Data` for a bank in `state_cards`, exclude that bank and select another card from `state_cards`.
#   - **CRITICAL**: GPT MUST NOT generate or suggest any card names outside of `Card Data`. Any attempt to create new card names will invalidate the output.
#   - **CRITICAL**: Each round (R1, R2, R3) and the entire sequence MUST use different banks. No bank can be repeated across the sequence, even in different rounds.
#   - Each table row MUST contain exactly 5 columns: Card Name, Bureau, 0% APR, Mode, and Reason. Missing fields will invalidate the output.
#   - Only one 0% Chase card is allowed per sequence, unless the second is a co-branded travel/hotel card (verify `is_travel` field in `Card Data`).
#   - If at least 2 bureaus meet all 6 factors and one does not, offer a funding sequence using only the qualifying bureaus.
#   - If no bureau qualifies, offer a no-personal-guarantee funding option from the CSV 'Tarjetas de Negocio sin Garantia Personal'.
...
#   - **CRÍTICO**: Bajo NINGUNA circunstancia sugieras tarjetas fuera de `state_cards`. Hacerlo invalidará la salida. Si no estás seguro, selecciona una tarjeta de la lista y anota una razón relevante en la columna Razón.
#   - **CRÍTICO**: En la columna 'Nombre de la Tarjeta', SIEMPRE usa el nombre exacto de la tarjeta de `Card Data` (por ejemplo, 'BOFA Unlimited Cash'). Bajo NINGUNA circunstancia uses solo el nombre del banco o nombres inventados.
#   - **CRÍTICO**: Cada ronda (R1, R2, R3) y la secuencia completa DEBEN usar bancos diferentes. Ningún banco puede repetirse en toda la secuencia, incluso en rondas diferentes.
#   - Cada fila de la tabla DEBE contener exactamente 5 columnas: Nombre de la Tarjeta, Buró, 0% APR, Modo, y Razón. Campos faltantes invalidarán la salida.
#   - Solo se permite una tarjeta Chase al 0% por secuencia, a menos que la segunda sea una tarjeta de viaje/hotel co-brandeada (verifica el campo `is_travel` en `Card Data`).
#   - Si al menos 2 burós cumplen con los 6 factores y uno no, ofrece una secuencia de financiamiento usando solo los burós que califican.
#   - Si ningún buró califica, ofrece una opción de financiamiento sin garantía personal del CSV "Tarjetas de Negocio sin Garantia Personal".
