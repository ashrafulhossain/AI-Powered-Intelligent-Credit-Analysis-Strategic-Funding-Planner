...
# Generate a table of revolving credit details based on the actual report data. Always extract the data, and if no matching data is found, use 'Data not available'.
...
# Always extract the revolving credit details from the report and present them in a structured table. If any data is missing or not found, clearly show 'Data not available' for that field.
...
# Ensure all available revolving credit data is extracted from the report and displayed in a table. For any field that is not found, default to 'Data not available'.
...
#             model="gpt-4-turbo",
#             messages=[
#                 {"role": "system", "content": "You are a strict, accurate AI financial credit analyst from Negocio Capital. You must extract all data exactly as available in the text. Do NOT hallucinate. Do NOT say 'Data not available' unless data is truly missing. Always follow instructions strictly. Be formal and expert in tone."},
